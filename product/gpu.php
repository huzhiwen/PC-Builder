<?php session_start();?>
 ...
 <div align="center">
 	<header id="header" class="skels-layers-fixed">
 		<h1><strong><a href="#">GPU</a></strong> </h1>
 		<nav id="nav">
 			<ul>
 ...
 <form  method="post" action="gpu.php" id="searchform"> <br>
  ...
  <fieldset>
 	...
 	<input style="margin-left:3em" type="checkbox" name="manu[0]" value="Sapphire" id="Sapphire" <?php if(isset($_POST['manu']) && is_array($_POST['manu']) && in_array('Sapphire', $_POST['manu'])) echo 'checked="checked"'; ?>/>
...
if(!isset($_POST['search']))
{
   ...
   // echo "<td>".$row["capacity"]."</td>";
		echo "<td> <form  method=\"post\" action= \"gpu.php#searchform".$i."\" id=\"searchform".$i."\">";
...
else
{
	$manus = $_POST['manu'];
	$nums = $_POST['num'];
	$str = "CREATE VIEW GPU_VIEW AS SELECT * FROM GPU ";
	$N = count($manus);
	$M = count($nums);
  ...
  if ($N != 0 || $M != 0)
	 ...
	 mysql_query($str);
  $query = mysql_query("SELECT * FROM GPU_VIEW;");
...
//      echo "<td>".$row["capacity"]."</td>";
	...
	echo "</tbody> </table>";
  mysql_query("DROP VIEW GPU_VIEW;");
}
...
function search()
{
  $response = exec('python query_sender.py '.$_POST['user_text']);
	...
	for ($i = 0; $i < $len; $i++)
	{
		...
		$string = "SELECT * FROM  GPU_VIEW WHERE manufacturer ='".$pieces[0]."' AND model_name = '".$pieces[1]."';";
		// echo $string;
		$query = mysql_query($string);
